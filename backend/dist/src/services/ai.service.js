...
class AiService {
     ...
     */
    static async handleAiAutoResponse(conversationId, customerMessageText) {
        try {
            ...
            const recheckedConv = await prisma_1.prisma.conversation.findUnique({
                where: { id: conversationId }
            });
            ...
            if (conversation.contact.phone) {
                try {
                    if (channel.type === 'EVOLUTION' && channel.evolutionInstanceName) {
                        await evolution_service_1.EvolutionService.sendTextMessage(channel.evolutionInstanceName, conversation.contact.phone, aiResponseText);
                    }
                    else if (channel.type === 'META_CLOUD' && channel.metaPhoneNumberId && channel.metaToken) {
                        await meta_service_1.MetaService.sendTextMessage(channel.metaPhoneNumberId, channel.metaToken, conversation.contact.phone, aiResponseText);
                    }
                }
                catch (dispatchError) {
            ...
            const updatedConv = await prisma_1.prisma.conversation.update({
                where: { id: conversation.id },
                data: { updatedAt: new Date() },
                include: { contact: true, channel: true, department: true, agent: true }
            });
            (0, socket_1.emitToWorkspace)(conversation.workspaceId, 'message:new', {
                message: aiMessage,
                conversationId: conversation.id
            });
            (0, socket_1.emitToWorkspace)(conversation.workspaceId, 'conversation:updated', {
                conversationId: conversation.id,
