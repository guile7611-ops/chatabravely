...
const actions = {
  ...
  fetchAllConversations: async ({ commit, state, dispatch }) => {
    ...
    if (
      paramsHash === lastFailedParamsHash &&
      (now - lastFailedTime < 5000 || failedFetchCount >= 3)
    ) {
      commit(types.CLEAR_LIST_LOADING_STATUS);
      return;
    }

    if (!state.allConversations.length) {
  ...
  fetchFilteredConversations: async ({ commit, state, dispatch }, params) => {
    ...
    if (
      filterHash === lastFailedFilterHash &&
      (now - lastFailedFilterTime < 5000 || failedFilterCount >= 3)
    ) {
      commit(types.CLEAR_LIST_LOADING_STATUS);
      return;
    }
    ...
    try {
      ...
      failedFilterCount += 1;
      lastFailedFilterHash = filterHash;
      lastFailedFilterTime = Date.now();
      console.warn(
        '[Filter API 500 Circuit Breaker] Interrompendo chamadas em loop de filtros:',
        filterHash
      );
    } finally {
      commit(types.CLEAR_LIST_LOADING_STATUS);
