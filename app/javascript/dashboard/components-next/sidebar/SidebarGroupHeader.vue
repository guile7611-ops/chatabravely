...
const showBadge = useMapGetter(props.getterKeys?.badge, false);
const dynamicCount = useMapGetter(props.getterKeys?.count, 0);
const count = computed(() => {
  const val = Number(dynamicCount.value || 0);
...
<template>
  <component
    ...
    class="flex items-center gap-2.5 px-2 py-1 rounded-lg h-9 min-w-0"
    role="button"
    draggable="false"
    :to="to"
    :title="label"
  ...
  >
    ...
    >
      <span
        ...
        :class="{
          'font-semibold text-base': isActive || hasActiveChild,
        }"
      >
        {{ label }}
      </span>
