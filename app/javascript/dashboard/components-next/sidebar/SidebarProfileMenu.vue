  ...
  >
    <template #trigger="{ toggle, isOpen }">
      ...
      >
        <Avatar
          :size="32"
          :name="currentUser?.name || currentUser?.available_name || 'Guilherme Tenorio'"
          :src="currentUser?.avatar_url || ''"
          :status="currentUserAvailability || 'online'"
          class="flex-shrink-0"
        />
        <div v-if="!isCollapsed" class="min-w-0">
          <div class="text-sm font-medium leading-4 truncate text-n-slate-12">
            {{ currentUser?.name || currentUser?.available_name || 'Guilherme Tenorio' }}
          </div>
          <div class="text-xs truncate text-n-slate-11">
            {{ currentUser?.email || 'guilherme.tenorio@multione.com' }}
          </div>
        </div>
      </button>
    </template>
