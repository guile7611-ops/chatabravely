...
function updateAssigneeTab(selectedTab) {
  if (activeAssigneeTab.value !== selectedTab) {
    ...
    // A troca de fila filtra somente a lista. A conversa selecionada continua
    // aberta até que o usuário escolha outra conversa ou a feche.
...
watch(activeTeam, (newVal, oldVal) => {
...
watch(conversationFilters, (newVal, oldVal) => {
  if (JSON.stringify(newVal) !== JSON.stringify(oldVal)) {
    const currentStoreFilters = store.getters.getConversationFilters;
    if (JSON.stringify(newVal) !== JSON.stringify(currentStoreFilters)) {
...
<template>
  <div
    class="flex flex-col flex-shrink-0 conversations-list-wrap bg-n-surface-1 relative ltr:border-r rtl:border-l border-n-weak"
    :class="[
