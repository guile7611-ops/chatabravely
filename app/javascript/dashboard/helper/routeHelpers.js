...
export const defaultRedirectPage = (to, permissions) => {
  const accountId = to?.params?.accountId || 1;
...
const validateActiveAccountRoutes = (to, user) => {
  const accountId = to?.params?.accountId || 1;
  const accountDashboardURL = `accounts/${accountId}/dashboard`;

  if (to.name === 'account_suspended') {
    return accountDashboardURL;
  }

  const userPermissions = getUserPermissions(user, accountId);
...
export const validateLoggedInRoutes = (to, user) => {
  const accountId = Number(to?.params?.accountId || 1);
  const currentAccount = getCurrentAccount(user, accountId) || { status: 'active' };

  if (!currentAccount) {
    return `accounts/1/dashboard`;
  }
  ...
  if (to.name !== 'account_suspended') {
    return `accounts/${accountId}/suspended`;
  }
